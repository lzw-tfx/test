...
class VisitSurveyDialog(QDialog):
    def __init__(self, db_manager, parent=None, record_data=None, youth_data=None):
        ...
        if record_data:
            ...
            self.load_youth_info_from_data()
    
    def init_ui(self):
        title = "编辑走访调查记录" if self.record_data else "添加走访调查记录"
        ...
        if self.youth_data:
            self.gender_input.setText(self.youth_data.gender)
    ...
    def load_youth_options(self):
        ...
        try:
            ...
            for option in youth_options:
                ...
                self.youth_data_dict[id_card] = {'name': name, 'gender': gender}
            ...
            # 只在主界面模式下设置下拉框选项
            if hasattr(self, 'name_combo') and hasattr(self, 'id_card_combo'):
    ...
    def load_youth_info(self):
        """加载指定青年的信息"""
        if self.youth_id_card and self.youth_id_card in self.youth_data_dict:
            youth_info = self.youth_data_dict[self.youth_id_card]
            if hasattr(self, 'name_combo'):
                self.name_combo.setCurrentText(youth_info['name'])
            if hasattr(self, 'id_card_combo'):
                self.id_card_combo.setCurrentText(self.youth_id_card)
            self.gender_input.setText(youth_info['gender'])
    
    def load_youth_info_from_data(self):
        """从青年数据对象加载信息（青年详情界面使用）"""
        if self.youth_data:
            # 青年数据已经在UI初始化时设置，这里不需要额外操作
            pass
    
    def on_name_changed(self, name):
        """姓名改变事件"""
        if hasattr(self, 'name_to_id_card') and name in self.name_to_id_card:
            id_card = self.name_to_id_card[name]
            if hasattr(self, 'id_card_combo'):
                self.id_card_combo.setCurrentText(id_card)
            if hasattr(self, 'youth_data_dict') and id_card in self.youth_data_dict:
                self.gender_input.setText(self.youth_data_dict[id_card]['gender'])
    
    def on_id_card_changed(self, id_card):
        """身份证号改变事件"""
        if hasattr(self, 'youth_data_dict') and id_card in self.youth_data_dict:
            name = self.youth_data_dict[id_card]['name']
            if hasattr(self, 'name_combo'):
    ...
    def load_record_data(self):
        """加载记录数据（编辑模式）"""
        if self.record_data:
            # record_data格式: (id, youth_id_card, youth_name, gender, survey_date, thoughts, spirit, created_at)
            
            # 设置日期
            if self.record_data[4]:  # survey_date
                try:
            ...
            # 加载图片
            try:
                self.image_data = self.db_manager.get_visit_survey_image(self.record_data[0])
                if self.image_data:
    ...
    def save_record(self):
        ...
        if self.youth_data:
            # 青年详情界面模式 - 数据已固定
            youth_name = self.name_input.text()
            youth_id_card = self.id_card_input.text()
        else:
            # 主界面模式 - 需要验证选择
            if not hasattr(self, 'name_combo') or not self.name_combo.currentText():
                QMessageBox.warning(self, "错误", "请选择姓名")
                return
            
            if not hasattr(self, 'id_card_combo') or not self.id_card_combo.currentText():
        ...
        try:
            ...
            if self.record_data:
                # 更新现有记录
                self.db_manager.update_visit_survey(
                    self.record_data[0], youth_id_card, youth_name, gender,
                    survey_date, self.image_data, thoughts, spirit
                )
                QMessageBox.information(self, "成功", "记录更新成功！")
            else:
                # 插入新记录
                self.db_manager.insert_visit_survey(
