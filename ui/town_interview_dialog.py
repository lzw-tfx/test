...
class TownInterviewDialog(QDialog):
    ...
    def init_ui(self):
        title = "编辑镇街谈心谈话记录" if self.record_data else "添加镇街谈心谈话记录"
    ...
    def load_record_data(self):
        ...
        if self.record_data:
            ...
            # 加载图片数据
            try:
                self.image_data = self.db_manager.get_town_interview_image(self.record_data[0])
                if self.image_data:
    ...
    def save_record(self):
        ...
        try:
            ...
            if self.record_data:
                # 更新现有记录
                self.db_manager.update_town_interview(
