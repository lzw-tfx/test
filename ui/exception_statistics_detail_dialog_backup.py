...
from PyQt6.QtCharts import QChart, QChartView, QLineSeries, QValueAxis, QDateTimeAxis, QCategoryAxis
...
class ExceptionStatisticsDetailDialog(QDialog):
    ...
    def create_individual_charts(self, data_sources, time_range):
        ...
        try:
            ...
            filtered_dates = []
            filtered_date_objects = []
            for date_str in sorted(all_dates):
                if start_date <= date_str <= end_date:
                    try:
                        date_obj = datetime.strptime(date_str, '%Y-%m-%d')
                        filtered_dates.append(date_str)
                        filtered_date_objects.append(date_obj)
                    except ValueError:
                        continue
            
            # 使用过滤后的日期
            dates = filtered_dates
            date_objects = filtered_date_objects
            ...
            for source_key, chart_info in self.chart_views.items():
                ...
                for i, (date_str, date_obj) in enumerate(zip(dates, date_objects)):
                    ...
                    # 使用实际日期对象生成时间戳
                    timestamp = QDateTime(date_obj).toMSecsSinceEpoch()
                    series.append(timestamp, exception_value)
                
                chart.addSeries(series)
                
                # 启用鼠标悬停显示日期和状态
                series.hovered.connect(lambda point, state, s=series, sk=source_key: 
                                     self.on_point_hovered(point, state, s, sk))
                ...
                # 设置日期范围
                if date_objects:
                    start_datetime = QDateTime(date_objects[0])
                    end_datetime = QDateTime(date_objects[-1])
                    axis_x.setRange(start_datetime, end_datetime)
                    
                    # 设置刻度数量
                    axis_x.setTickCount(min(len(dates), 8))
    ...
    def create_individual_charts_by_date_range(self, data_sources, start_date, end_date):
        ...
        try:
            ...
            while current_date <= end_datetime:
                ...
                base_date_objects.append(current_date)
            ...
            for source_key, chart_info in self.chart_views.items():
                ...
                # 设置日期范围
                if date_objects:
                    start_qdatetime = QDateTime(date_objects[0])
                    end_qdatetime = QDateTime(date_objects[-1])
                    axis_x.setRange(start_qdatetime, end_qdatetime)
